...
fn main() {
    ...
    for n in iterations {
        ...
        for _ in 0..n {
            let _ml_result = Bn254::multi_miller_loop(vec![g1_prepared.clone()], vec![g2_prepared.clone()]);
        }
        let miller_duration = miller_start.elapsed();
        
        println!("Total duration for {} iterations: {:?}", n, miller_duration);
        println!("Miller loops per second: {:.0}", n as f64 / miller_duration.as_secs_f64());
        println!("Average time per Miller loop: {:.2} μs", miller_duration.as_micros() as f64 / n as f64);
        
        // Do one final exponentiation for reference
        if n == 1 {
            let ml_result = Bn254::multi_miller_loop(vec![g1_prepared.clone()], vec![g2_prepared.clone()]);
            ...
            println!("Miller loop vs Final exp ratio: {:.2}:1", 
