use dory::curve::{test_rng, G2AffineWrapper, ArkBn254Pairing};
...
fn main() {
    ...
    for size in sizes {
        ...
        let g1_points: Vec<G1Affine> = (0..size)
        ...
        println!("Pairings per second: {:.0}", size as f64 / duration.as_secs_f64());
        println!("Average time per pairing: {:.2} μs", duration.as_micros() as f64 / size as f64);
        ...
        if size <= 100 {
            ...
            println!("Speedup: {:.2}x", individual_duration.as_secs_f64() / duration.as_secs_f64());
        }
    }
}