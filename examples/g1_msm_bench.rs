...
use dory::arithmetic::{Group, Field, MultiScalarMul};
...
fn main() {
    ...
    println!("Elements per second: {:.0}", size as f64 / duration.as_secs_f64());
    println!("Average time per element: {:.2} ns", duration.as_nanos() as f64 / size as f64);
}